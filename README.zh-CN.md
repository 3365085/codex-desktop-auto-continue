...
内部重连耗尽而正式失败时，监视器会向原来的线程排入一条 `continue`。
...
本机 JSONL 会话日志
        ...
        │ 读取准确的原 thread ID
        ▼
codex queue --thread <原ID> --message continue
        │
        ▼
原来的 Desktop 对话继续工作
```
...
仍然失败并产生 `task_complete.error`，才向原线程排入 `continue`。
...
--codex-bin PATH          用于执行 `codex queue` 的程序
