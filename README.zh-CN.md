...
如果本轮暂时性失败让 Goal 变成 Desktop 中显示的“目标已停滞”（app-server
状态为 `blocked`），监视器会像顶部“继续”按钮一样把它恢复为 `active`。
如果 app-server 调用失败，活动或这种暂时阻塞的 Goal 才会回退到同线程
`continue`，优先保证任务还能向前推进；日志会明确记录这次回退。手动暂停、
完成或受用量限制的 Goal 不会被监视器擅自恢复。
